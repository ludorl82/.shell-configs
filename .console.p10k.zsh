...
() {
  ...
  ##########################[ overrides: match previous agnoster prompt ]##########################
  # Single line, matching the old agnoster layout: context -> dir -> git (left),
  # background jobs/status/virtualenv (right).
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(my_context dir vcs)
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(virtualenv background_jobs status)

  # We don't use the instant-prompt snippet at the top of .zshrc, so disable it
  ...
  # status: red cross on error, black bg (agnoster convention)
  typeset -g POWERLEVEL9K_STATUS_OK=false
  typeset -g POWERLEVEL9K_STATUS_ERROR_BACKGROUND=0
  typeset -g POWERLEVEL9K_STATUS_ERROR_FOREGROUND=1
  typeset -g POWERLEVEL9K_STATUS_ERROR_VISUAL_IDENTIFIER_EXPANSION=$'×'

  # background jobs: cyan gear, black bg (agnoster convention)
  typeset -g POWERLEVEL9K_BACKGROUND_JOBS_VERBOSE=false
  typeset -g POWERLEVEL9K_BACKGROUND_JOBS_BACKGROUND=0
  typeset -g POWERLEVEL9K_BACKGROUND_JOBS_FOREGROUND=6
  typeset -g POWERLEVEL9K_BACKGROUND_JOBS_VISUAL_IDENTIFIER_EXPANSION=$'ö'

  # virtualenv: black bg, default fg
  typeset -g POWERLEVEL9K_VIRTUALENV_BACKGROUND=0
  typeset -g POWERLEVEL9K_VIRTUALENV_FOREGROUND=7

  # Custom segment replacing the old prompt_context: username + a random decorative
  # glyph (personality touch carried over from agnoster) + time. Black bg like the
  # original; red bg + lightning icon when running as root.
  function prompt_my_context() {
    ...
    if [[ $UID -eq 0 ]]; then
      p10k segment -b 1 -f 0 -i '⚡' -t "%n ${glyph} %D{%T}"
    else
      p10k segment -b 0 -f 7 -t "%n ${glyph} %D{%T}"
    fi
  }
}
